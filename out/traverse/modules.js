    ...
    switch (property.key.name) {
      case 'state':
        if (property.shorthand) {
          ...
          if (m2pmap[value.name]) {
            let { export: importState, lineOfFile } = state_1.walkFile(
              cwf,
              m2pmap[value.name],
            );
      ...
      case 'getters':
        if (property.shorthand) {
          let value = property.value;
          if (m2pmap[value.name]) {
            let { export: importGetters, lineOfFile } = state_1.walkFile(
              cwf,
              m2pmap[value.name],
            );
            infoObj.getters = getters_1.parseGetters(importGetters, lineOfFile);
          } else if (defmap[value.name]) {
            infoObj.getters = getters_1.parseGetters(
              defmap[value.name],
              lineOfFile,
            );
          }
      ...
      case 'mutations':
        if (property.shorthand) {
          ...
          if (m2pmap[value.name]) {
            let {
              export: importGetters,
              lineOfFile,
            } = mutations_1.walkMutationsFile(cwf, m2pmap[value.name]);
            infoObj.mutations = mutations_1.parseMutations(
              importGetters,
              lineOfFile,
            );
          } else if (defmap[value.name]) {
